import { Component, ElementRef, EventEmitter, HostListener, Input, Output, ViewChild, signal, effect } from '@angular/core';
  ...
  template: `
    <div class="relative w-full">
      ...
      <div class="relative">
        ...
        >
          <!-- Selected items -->
           @if (!hideSelectedItemsWithinSearchInput) {
          @for (item of selectedItems(); track item.id) {
            <div class="flex items-center bg-primary-100 text-primary-800 rounded-full py-1 pl-3 pr-2 text-sm">
              @if (item.avatar) {
                <img [src]="item.avatar" class="w-5 h-5 rounded-full me-2" [attr.alt]="item.title">
              }
          ...
          <!-- Just shows search icon and placeholder -->
          ...
          <input
            ...
            [placeholder]="selectedItems().length === 0 || hideSelectedItemsWithinSearchInput ? placeholder : ''"
            [(ngModel)]="searchQuery"
            (input)="filterItems()"
            (focus)="openDropdown()"
        ...
        >
          ...
          @if (selectedItems().length > 0) {
            <div class="p-2 border-t border-primary-100 text-xs text-gray-500 bg-primary-50">
              {{ selectedItems().length }} {{ 'selected' | translate }}
            </div>
          }
        </div>
      }
...
export class MultiItemsSelectorComponent {
  ...
  @Input() hideSelectedItemsWithinSearchInput? = false;
  ...
  constructor() {
    effect(() => {
      this.dropdownVisible = this.isOpen();  // Directly tie to isOpen for smooth transitions
      if (this.isOpen()) {
  ...
  onClickOutside(event: Event) {
    ...
    const clickedInside = this.searchInput?.nativeElement.contains(target) || 
                         target.closest('.absolute.z-10'); // dropdown menu
  ...
  get paginatedItems(): Item[] {
    ...
    return this.items; // for server-side pagination, items is the current page
  }
  ...
  removeItem(item: Item, event?: Event) {

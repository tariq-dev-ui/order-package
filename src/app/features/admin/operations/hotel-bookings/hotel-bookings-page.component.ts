...
@Component({
  selector: 'app-hotel-bookings-page',
  standalone: true,
  ...
  template: `
    <div class="operations-page relative">
      ...
      <general-voucher-table
        [typeIds]="typeIds"
        [vouchers]="vouchers"
        [voucherActionsList]="voucherActionsList"
        (refreshVouchers)="refreshVouchersList()" />
      <pagination [currentPage]="page()" [totalPages]="totalPages()" (pageChange)="setPage($event)" />
    </div>
  `,
  styles: [`
    .operations-page { position: relative; display: flex; flex-direction: column; gap: 24px; }
  `],
})
export class HotelBookingsPageComponent {
  ...
  private loadVouchers(): void {
    ...
    this.voucherService.getVouchersForOperationApproval({
      pageIndex,
      pageSize: this.pageSize,
      typeid: this.typeIds[0],
  ...
  private loadCount(): void {
    this.voucherService.getVouchersForOperationApprovalCount({
      typeid: this.typeIds[0],
      agentId: this.agentId(),
    }).subscribe((count) => {
      this.totalPages.set(Math.max(1, Math.ceil(count / this.pageSize)));
    });
  }
}
